							...
							<table width="1000">
								<tr>
									<td align="right" bgcolor="#b0e0e6"><a
										href="/Project/writeForm.do">글쓰기</a></td>
								</tr>
							</table>
							<!-- 데이터의 유무 -->
...
<%-- 								
 ...
 --%>
								...
								<!-- 글상세보기 부분  --> 
								<a href="/Project/content.do?r_num=${article.r_num}&pageNum=${pgList.currentPage}">${article.r_title}</a>
				...
				<a
					href="/Project/list.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}">[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${pgList.startPage}"
				end="${pgList.endPage}">
				<a
					href="/Project/list.do?pageNum=${i}&search=${search}&searchtext=${searchtext}">
			...
			<form name="test" action="/Project/review.do">
				<select name="search">
