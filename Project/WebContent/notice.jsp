...
<!--  -->               
					...
					<c:forEach var="article" items="${articleList}">
						<tr height="30">
							...
							<td align="center" width="50"><c:out value="${article.n_num}" /> </td>
							<td width="250">
...
<%-- 								
 ...
 --%>
								...
								<!-- 글상세보기 부분  --> 
								<a href="/Project/noticecontent.do?n_num=${article.n_num}&pageNum=${pgList.currentPage}">${article.n_title}</a>
								<c:if test="${article.n_view>=20}">
									<img src="images/hot.gif" border="0" height="16">
							</td>
							</c:if>
							<td align="center" width="100">${article.a_id}</td>
							<%-- <td align="center" width="150"><fmt:formatDate value="${article.r_date}" timeStyle="medium" pattern="yy.MM.dd (hh:mm)" /></td> --%>
							<td align="center" width="150">${article.n_date}</td>
							<td align="center" width="50">${article.n_view}</td>
						...
						<table class="table table-bordered table-hover table-condensed">
							<tr>
								<td align="right" bgcolor="#fff">
									<input type="button" class="btn btn-custom min-width" value="글쓰기" OnClick="window.location='/Project/writeNoticeForm.do'">
								</td>
							</tr>
						</table>
				...
				<a
					href="/Project/notice.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}">[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${pgList.startPage}"
				end="${pgList.endPage}">
				<a
					href="/Project/notice.do?pageNum=${i}&search=${search}&searchtext=${searchtext}">
			...
			<c:if test="${pgList.endPage < pgList.pageCount}">
				<a href="/Project/notice.do?pageNum=${pgList.startPage+pgList.blockSize}&search=${search}&searchtext=${searchtext}">[다음]</a>
			</c:if>
</form>
			<p>
				<!-- 검색어 추가(자주 검색이 되는 항목을 잘 선택) : 제목, 작성자, 제목+본문 -->
			<form name="test" align="center" action="/Project/notice.do">
				<select name="search">
